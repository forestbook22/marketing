...
public class TitlesEditServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Title ti = em.find(Title.class, Integer.parseInt(request.getParameter("id")));

        em.close();

        User login_user = (User)request.getSession().getAttribute("login_user");
        if(ti != null && login_user.getId() == ti.getUser().getId()) {
            request.setAttribute("title", ti);
            request.setAttribute("_token", request.getSession().getId());
            request.getSession().setAttribute("title_id", ti.getId());
        }
