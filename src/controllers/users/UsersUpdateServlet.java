...
public class UsersUpdateServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //元々あるユーザーネームと異なるネームが入力されていたら重複チェックを指定
        ...
        //元々あるパスワードと異なるパスワードが入力されていたら重複チェックを指定
        ...
        u.setAdmin_flag(Integer.parseInt(request.getParameter("admmin_flag")));
        u.setUpdated_at(new Timestamp(System.currentTimeMillis()));
        u.setDelete_flag(0);

        List<String> errors = UserValidator.validate(u, name_duplicate_check_flag, password_check_flag);
        if(errors.size() > 0){
            em.close();
